...
pageAdditionalFields = _container_;
...
require([
    ...
    "ingrid/dialog"
        ...
        pageAdditionalFields.checkBoxLegacyVisibility = function() {
         ...
         */
         pageAdditionalFields.hideEmtyRubrics = function() {
        ...
        function reload(fromServer, /*Array*/rubrics, pos) {
            if (fromServer) {
                ...
                pageAdditionalFields.checkBoxLegacyVisibility();
                pageAdditionalFields.hideEmtyRubrics();
        ...
        function writeControl(control, putHere, editFunction, deleteFunction, countControl, firstControl, lastControl, newControl) {
        	...
        	//outerDiv.connect(outerDiv, "onMouseEnter", showButtons);
        	//outerDiv.connect(outerDiv, "onMouseLeave", hideButtons);
        	if(countControl % 2 == 0){
        ...
        function appendJsInfo(appendTo, control) {
            if (control.scriptedProperties && lang.trim(control.scriptedProperties).length > 0) {
                ...
                var tip = new Tooltip({connectId:[tipId], label:"<code class='javascript'>"+/*convertToHTML(*/control.scriptedProperties.replace(/\n/g, "<br>")/*)*/+"</code>"});
        ...
        function appendIdfInfo(appendTo, control) {
            if (control.scriptedCswMapping && lang.trim(control.scriptedCswMapping).length > 0) {
                ...
                var tip = new Tooltip({connectId:[tipId], label:"<code class='javascript'>"+/*convertToHTML(*/control.scriptedCswMapping.replace(/\n/g, "<br>")/*)*/+"</code>"});
                on(tip, "onShow", function() { query("code").forEach(dojox.highlight.init);});
        ...
        function createButtons(item, whereInDOM, editFunction, deleteFunction, firstItem, lastItem, newControl) {
            ...
            if (!external) {
        		var editFunction = editElement;
        		var deleteFunction = deleteElement;
        	}
        ...
        function moveUp(nodeToMove) {
        	...
        	doActionOnData(nodeToMove, moveUp);
        }
        ...
        function moveDown(nodeToMove) {
            ...
            doActionOnData(nodeToMove, moveDown);
        }
        ...
        function changeCheckBox(id) {
           	if (registry.byId(id).checked) {
           		registry.byId(id).setValue(false);
            }else{
            	registry.byId(id).setValue(true);
            }
        }
        
        pageAdditionalFields.toggleRubric = function(rubricId, mode) {
        ...
        pageAdditionalFields.toggleExpandAllRubrics = function() {
            var mode = registry.byId("checkBoxExpandAllRubrics").get("checked") ? "expand" : "collapse";
            console.debug("mode: " + mode);
            var allRubrics = query(".rubricDiv");
            array.forEach(allRubrics, function(rubric) {
                pageAdditionalFields.toggleRubric(rubric.parentNode.id, mode)
            });
        };
        
        function convertToHTML(noBreaks) {
        ...
        <div data-dojo-type="dijit/layout/ContentPane" region="top" style="height:60px;">
            <div id="formFieldsToolBar">
            	<div style="float:left; padding: 2px 10px 0;">
	            	<div id="checkBoxLegacyFieldsButton" data-dojo-type="dijit/form/CheckBox" name="<fmt:message key='dialog.admin.additionalfields.hide.legacy' />" 
					       type="checkbox">
					       <script type="dojo/method" event="click" args="evt">
							pageAdditionalFields.checkBoxLegacyVisibility(); 
							</script>      
					</div>
				...
				<div style="float:left; padding: 2px 10px 0;">
					<div id="checkBoxHideRubricsButton" data-dojo-type="dijit/form/CheckBox" name="<fmt:message key='dialog.admin.additionalfields.hide.empty.rubrics' />" 
					       type="checkbox">
					       <script type="dojo/method" event="click" args="evt">
							pageAdditionalFields.hideEmtyRubrics(); 
							</script> 
                ...
                <div style="float:left; padding: 2px 10px 0;">
                    <div id="checkBoxExpandAllRubrics" data-dojo-type="dijit/form/CheckBox" checked name="<fmt:message key='dialog.admin.additionalfields.expand.all.rubrics' />" 
                           type="checkbox">
                           <script type="dojo/method" event="click" args="evt">
                            pageAdditionalFields.toggleExpandAllRubrics(); 
                            </script> 
                    </div>
