function displayErrorMessage(err) {
    ...
    if (err && err.message) {
        ...
        /*while(typeof(errorMessage) == "object") {
            console.debug("Error message is obj. Extracting...");
			if (err.message != "")
                errorMessage = err.message;
			else {
				errorMessage = err.javaClassName;
				if (err.cause) {
		            errorMessage += "\n" + err.message; 
		        }
				errorMessage += printStackTrace(err);
			}
			    
            console.debug("err: "+err + " message: "+err.message);
        }*/
        ...
        dialog.show(message.get("general.error"), message.get("dialog.undefinedError") + ": " + err, dialog.WARNING);
    }
}
...
function printStackTrace(error) {
	...
	dojo.forEach(error.stackTrace, function(stackPart) {
