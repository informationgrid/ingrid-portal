...
//	dojo.debugShallow(nodeData);
	...
	var superiorElements = renderSuperiorElements(dojo.widget.byId("tree").selectedNode.parent);
	renderList(superiorElements.split("@@"), message.get("ui.obj.general.superior.objects"));//, rowProperty, renderFunction)
	
	// add subordinated elements
	var deferred = renderSubordinatedElements(dojo.widget.byId("tree").selectedNode);
	deferred.addCallback(function(res) {
		var sortedList = res.split("@@").sort(function(a,b) {return UtilString.compareIgnoreCase(a,b);});
		renderList(sortedList, message.get("ui.obj.general.subordinated.objects"));
	});
...
//			renderTextWithTitle(nodeData.ref1SpatialSystem, message.get("ui.obj.type1.spatialSystem"));
		    ...
		    renderTextWithTitle(nodeData.ref3HasAccessConstraint ? message.get("general.yes"): message.get("general.no"), message.get("ui.obj.type3.ref3HasAccessConstraint.title"));
			...
			renderTextWithTitle(dojo.widget.byId("ref6ServiceType")._getDisplayValueForValue(nodeData.ref6ServiceType), message.get("ui.obj.type6.serviceType"));
			renderList(nodeData.ref6ServiceVersion, message.get("ui.obj.type6.serviceVersion"));
			renderTextWithTitle(nodeData.ref6SystemEnv, message.get("ui.obj.type6.environment"));
			renderTextWithTitle(nodeData.ref6History, message.get("ui.obj.type6.history"));
			renderTextWithTitle(nodeData.ref6BaseDataText, message.get("ui.obj.type6.generalDataTable.title") + " (" + message.get("ui.obj.type6.generalDataTable.tab.text") + ")");
			renderTable(nodeData.ref6UrlList, ["name", "url", "urlDescription"], [message.get("ui.obj.type6.urlList.header.name"), message.get("ui.obj.type6.urlList.header.url"), message.get("ui.obj.type6.urlList.header.urlDescription")], message.get("ui.obj.type6.urlList"));
		}
        ...
        renderTextWithTitle(nodeData.availabilityDataFormatInspire, message.get("ui.obj.availability.dataFormatInspire"));
		...
		renderList(sortedList, message.get("ui.adr.thesaurus.terms"), "title");
		sortedList = nodeData.thesaurusTopicsList.sort(function(a,b) {return UtilString.compareIgnoreCase(dojo.widget.byId("thesaurusTopicsCombobox")._getDisplayValueForValue(a),dojo.widget.byId("thesaurusTopicsCombobox")._getDisplayValueForValue(b));}); 
		renderList(sortedList, message.get("ui.obj.thesaurus.terms.category"), null, function (val) { return dojo.widget.byId("thesaurusTopicsCombobox")._getDisplayValueForValue(val);});
		sortedList = nodeData.thesaurusInspireTermsList.sort(function(a,b) {return UtilString.compareIgnoreCase(dojo.widget.byId("thesaurusInspireCombobox")._getDisplayValueForValue(a),dojo.widget.byId("thesaurusInspireCombobox")._getDisplayValueForValue(b));});
		renderList(sortedList, message.get("ui.obj.thesaurus.terms.inspire"), null, function (val) { return dojo.widget.byId("thesaurusInspireCombobox")._getDisplayValueForValue(val);});
		...
		if (nodeData.linksFromObjectTable.length > 0 || nodeData.linksToObjectTable.length > 0 || nodeData.linksToUrlTable.length > 0) {
	    	...
	    	renderList(sortedList, message.get("dialog.compare.object.linksFromTable.title"), "title")
	    	sortedList = nodeData.linksToObjectTable.sort(function(a,b) {return (b.title<a.title)-(a.title<b.title);});
	    	renderList(sortedList, message.get("dialog.compare.object.linksToTable.title"), "title")
...
function renderSubordinatedElements(node) {
	...
	TreeService.getAllSubTreeChildren(node.id, node.nodeAppType, {
	...
	deferred.addCallback(function(children) {
        ...
        for (var i=0; i<children.length; i++) {
            //dojo.debug("Got Children: " + children[i].title);
            subList = children[i].title + "@@" + subList;
        }
        //dojo.debug("return list: " + subList);
        return subList;
	});
