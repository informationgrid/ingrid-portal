...
Local = {
    ...
    localizedStrings: {
        ...
        tHintZoomToView : { en: 'Hint: You may have to zoom into the map to see the layers of the external service. The provider of the service is responsible for the display of the service. NUMIS has no influence here.', de: 'Hinweis: M&ouml;glicherweise m&uuml;ssen Sie die Ansicht vergr&ouml;&szlig;ern, um Ebenen von externen Kartendiensten betrachten zu k&oumlnnen. Der Betreiber des Kartendienstes ist f&uuml;r die Anzeige verantwortlich. NUMIS hat keine Beteiligung an dessen Verhalten.' },
        ...
        tWelcomeTextTransparency: { en: 'Apply layer <strong>transparency</strong> using the slider control', de: 'Steuert die Karten-Transparenz mithilfe eines Schiebereglers'},
        tWelcomeTextMetadata: { en: 'Display information (metadata) about selected map services or layers', de: 'Zeigt Informationen zu Kartendiensten oder Kartenebenen an'},
        tWelcomeTextExpand: { en: 'Open and close all layers in the tree structure by one click', de: 'a) Klappt alle Dienste eines <strong>Anbieters</strong> oder eines <strong>Themas</strong> auf / zu<br/>b) Klappt die Ebenen aller <strong>Aktiven Dienste</strong> auf / zu'},
        tWelcomeTextZoomLayerExtent: { en: 'Zoom to a layer’s visible area', de: 'Zoomt auf den sichtbaren Bereich eines Kartendienstes oder einer Karte'},
        
        tWelcomeTextInfo: { en: 'A click on the map reveals all available information for that location', de: 'Zeigt die Objektinformationen (Sachattribute) aller aktiven Kartenebenen an'},
        ...
        tWelcomeTextLoad: { en: 'Open an existing map (Access to this feature requires a personal account)', de: 'L&auml;dt die zuvor gespeicherte Karten (<strong>Mein NUMIS</strong> Login erforderlich)'},
        tWelcomeTextSave: { en: 'Saves the current map (Access to this feature requires a personal account)', de: 'Speichert die aktuelle Kartenansicht (<strong>Mein NUMIS</strong> Login erforderlich)'},
        ...
        tWelcomeTextTipp4: { en: '<h1>You are searching maps on a particular topic?</h1><p>Use the search field below the list of topics to find further maps. By entering a keyword, all connected catalogues from our partners will be searched for suitable maps (WMS).</p>', de: '<h1>Sie suchen Karten zu einem bestimmten Umweltthema?</h1><p>Finden Sie eine passende Karte mit Hilfe der Suchfunktion unterhalb der Anbieter- und Themenliste. Mit der Suchanfrage werden s&auml;mtliche angeschlossene Kataloge nach passenden Kartendiensten (WMS) durchsucht.</p>'},
        tWelcomeTextTipp5: { en: '<h1>You want to save the current map and load it at a later stage?</h1><p>Take advantage of a personal NUMIS Login: Register on <a href="/personalisierungsoptionen"><strong>my NUMIS</strong></a> and make use of various options to personalize NUMIS.</p>', de: '<h1>Sie wollen eine Kartenansicht speichern und zu einem sp&auml;teren Zeitpunkt wieder aufrufen?</h1><p>Nutzen Sie die Vorteile eines pers&ouml;nlichen NUMIS-Logins: Auf der Seite des Men&uuml;reiters <a href="/personalisierungsoptionen"><strong>Mein NUMIS</strong></a> k&ouml;nnen Sie sich als Benutzer registrieren. Damit steht Ihnen eine Vielzahl von M&ouml;glichkeiten offen, die Funktionen des Portals nach Ihren Bed&uuml;rfnissen zusammenzustellen.</p>'},
        tWelcomeTextTitle: { en: 'WELCOME TO THE MAP VIEWER', de: 'WILLKOMMEN BEIM KARTENVIEWER'},
        tWelcomeTextSubtitle: { en: 'Our map viewer contains a selection of digital maps (WMS) from public organisations in Germany. More than 200 digital maps are ordered by provider and by topic. Beyond that you can use the search function to find further maps.', de: 'Unser Kartenviewer enth&auml;lt eine Auswahl digitaler Kartendienste (WMS) von Beh&ouml;rden aus Niedersachsen. Sie haben die Wahl: Verschiedene Karten stellen wir &uuml;ber die Themen- und Anbieterliste bereit. Weitere Kartendienste k&ouml;nnen Sie über die Suchmaske recherchieren.'},
