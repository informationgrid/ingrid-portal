...
Local = {
    ...
    localizedStrings: {
        ...
        tWelcomeTextLoad: { en: 'Open an existing map (Access to this feature requires a personal account)', de: 'L&auml;dt die zuvor gespeicherte Karten (<strong>Mein GeoPortal.WSV</strong> Login erforderlich)'},
        tWelcomeTextSave: { en: 'Saves the current map (Access to this feature requires a personal account)', de: 'Speichert die aktuelle Kartenansicht (<strong>Mein GeoPortal.WSV</strong> Login erforderlich)'},
        ...
        tWelcomeTextTipp4: { en: '<h1>You are searching maps on a particular topic?</h1><p>Use the search field below the list of topics to find further maps. By entering a keyword, all connected catalogues from our partners will be searched for suitable maps (WMS).</p>', de: '<h1>Sie suchen Karten zu einem bestimmten Umweltthema?</h1><p>Finden Sie eine passende Karte in der Rubrik <b>Suche</b>. Mit der Suchanfrage werden s&auml;mtliche angeschlossene Kataloge nach passenden Kartendiensten durchsucht.</p>'},
        tWelcomeTextTipp5: { en: '<h1>You want to save the current map and load it at a later stage?</h1><p>Take advantage of a personal GeoPortal.WSV Login: Register on <a href="/personalisierungsoptionen"><strong>my GeoPortal.WSV</strong></a> and make use of various options to personalize GeoPortal.WSV.</p>', de: '<h1>Sie wollen eine Kartenansicht speichern und zu einem sp&auml;teren Zeitpunkt wieder aufrufen?</h1><p>Nutzen Sie die Vorteile eines pers&ouml;nlichen GeoPortal.WSV Logins: Unter <a href="/personalisierungsoptionen"><strong>Mein GeoPortal.WSV</strong></a> k&ouml;nnen Sie sich als Benutzer registrieren. Damit steht Ihnen eine Vielzahl von M&ouml;glichkeiten offen, die Funktionen des Portals nach Ihren Bed&uuml;rfnissen zusammenzustellen.</p>'},
        tWelcomeTextTitle: { en: 'WELCOME TO THE MAP VIEWER', de: 'WILLKOMMEN BEIM KARTENVIEWER'},
        tWelcomeTextSubtitle: { en: 'Our map viewer contains a selection of digital maps (WMS) from public organisations in Germany. More than 200 digital maps are ordered by provider and by topic. Beyond that you can use the search function to find further maps.<br/>The base map (web map service WebAtlasDE.light, &#169; GeoBasis-DE / BKG) is provided by the Federal Agency for Cartography and Geodesy (BKG).', de: 'Im GeoPortal.WSV-Kartenviewer werden eine Vielzahl verschiedener Kartendienste bereitgestellt.<br/>Die Hintergrundkarte (Kartendienst WebAtlasDE.light, &#169; GeoBasis-DE / BKG) wird durch das Bundesamt f&uuml;r Kartographie und Geod&auml;sie (BKG) bereitgestellt.'},
		...
		tPortal: {en: 'Open GeoPortal.WSV', de: 'GeoPortal.WSV &ouml;ffnen'},
