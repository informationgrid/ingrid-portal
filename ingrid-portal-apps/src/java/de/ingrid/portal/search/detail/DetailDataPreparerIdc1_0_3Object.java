...
public class DetailDataPreparerIdc1_0_3Object implements DetailDataPreparer {
    ...
    private final static Logger log = LoggerFactory.getLogger(DetailDataPreparerIdc1_0_3Object.class);
...
// tab "Availability"
	...
	private void addAdditionalInformation(List elements, Record record) {
		...
		if (listLegalReferences.size() > 0 || listXMLRecords.size() > 0 || UtilsVelocity.hasContent(metadataLanguage).booleanValue() || UtilsVelocity.hasContent(dataLanguage).booleanValue()
				|| UtilsVelocity.hasContent(datasetUsage).booleanValue() || UtilsVelocity.hasContent(infoNote).booleanValue() || (listConformity.size() > 0 && (record.getString("t01_object.obj_class").equals("3") 
						|| record.getString("t01_object.obj_class").equals("1")))) {
	...
	private void addSpatialReference(List elements, Record record) {
		...
		List refLocationRecords = getSubRecordsByColumnName(record, "location");
		...
		if (refRecords.size() > 0 || refLocationRecords.size() > 0 || UtilsVelocity.hasContent(locDescr).booleanValue() || UtilsVelocity.hasContent(extentMinimum).booleanValue()
