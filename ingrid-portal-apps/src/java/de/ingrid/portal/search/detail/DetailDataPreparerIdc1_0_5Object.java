...
public class DetailDataPreparerIdc1_0_5Object implements DetailDataPreparer {
	...
	private final static Logger		log	= LoggerFactory.getLogger(DetailDataPreparerIdc1_0_5Object.class);
...
// tab "Availability"
	...
	private void addAvailabilityInformation(List elements, Record record) {
		...
		if (listAvailFormat.size() > 0 || listMediaOption.size() > 0 || listObjectAccess.size() > 0 || UtilsVelocity.hasContent(orderingInstructions).booleanValue()) {
			
			addSectionTitle(elements, messages.getString("availability"));
			// add object access information (new for INSPIRE)
			if (listObjectAccess.size() > 0) {
				HashMap element = new HashMap();
				element.put("type", "table");
				...
				ArrayList body = new ArrayList();
				element.put("body", body);
				for (int i = 0; i < listObjectAccess.size(); i++) {
					Record tableRecord = (Record) listObjectAccess.get(i);
					ArrayList row = new ArrayList();
		...
		if (refRecords.size() > 0) {
			...
			if (refRecords.size() > 0) {
				...
				for (int i = 0; i < refRecords.size(); i++) {
					...
					if (((serviceTypeKey != null && serviceTypeKey.equals("2")) || (serviceType != null && serviceType.toLowerCase().indexOf("wms") != -1)) && serviceLinkRecords.size() > 0
				...
				for (int i = 0; i < wmsServiceLinks.size(); i++) {
					if (UtilsVelocity.hasContent(wmsServiceLinks.get(i)).booleanValue()) {
						if (UtilsVelocity.hasContent(wmsServiceLinks.get(i).trim()).booleanValue()) {
							...
							elementLink.put("href", "portal/main-maps.psml?wms_url=" + UtilsVelocity.urlencode(wmsServiceLinks.get(i).trim()));
