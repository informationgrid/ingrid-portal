...
public class UtilsMapServiceManager {
    ...
    private UtilsMapServiceManager() {
     ...
     * @throws Exception
     */
    public static void countTempServiceNumber() throws Exception {
        
        File path = new File(getTmpDirectory(getConfig().getString("temp_service_path", null)));
        ArrayList<Long> fileArray = new ArrayList<>();
        if (path.exists() && path.list().length > Integer.parseInt(getConfig().getString("temp_service_limit", null))) {
            ...
            for (int i = 0; i < files.length; i++) {
                if (files[i].lastModified() == fileArray.get(0) && files[i].delete() && log.isDebugEnabled()) {
     ...
     * @throws Exception
     */
    public static String createTemporaryService(String title, List<Map<String, String>> coords, String fileType) throws Exception {
        String fileName = "";
        ...
        if(fileType.endsWith(UtilsFileHelper.KML)){
            ...
            String path = url.getPath();
                ...
                UtilsFileHelper.writeContentIntoFile(getTmpDirectory(getConfig().getString("temp_service_path", null)).concat(fileName), mergeTemplateKML(path, coords, title));    
        } 
        return fileName;
    }
     ...
     * @throws Exception
     */
    public static String getTmpDirectory(String path) {
    ...
    public static String getKMLTmpDirectory() throws Exception {
        if(tmpDirectory == null){
    ...
    public static void setTmpDirectory(String tmpDirectory) {
        UtilsMapServiceManager.tmpDirectory = tmpDirectory;
    }
     ...
     * @throws Exception
     */
    public static String mergeTemplateKML(String path, List<Map<String, String>> coords, String title) throws Exception {
        ...
        try (BufferedReader templateReader = new BufferedReader(new InputStreamReader(new FileInputStream(templatePath), StandardCharsets.UTF_8))) {
            Velocity.init();
    ...
    public static Map<String, Object> createKmlFromIDF(String iPlugId, String documentId) throws Exception {
        ...
        if(node.getNamespaceURI().equals(IDFNamespaceContext.NAMESPACE_URI_KML)){
            ...
            if(documentNode != null){
                ...
                if(placemarkNodeList != null){
                    List<HashMap<String, String>> placemarks = new ArrayList<>();
                    for(int i=0; i < placemarkNodeList.getLength(); i++){
                        ...
                        if(pmPointNode != null && pmPointNode.hasChildNodes()){
     ...
     * @param unknown
     */
    public static void getCoordinatesDetailsFromRecord(Record record, List<HashMap<String, String>> coordList){
