-- DB Version erhöhen
UPDATE ingrid_lookup SET `item_value` = '3.2.0b', `item_date` = NOW() WHERE `ingrid_lookup`.`item_key` ='ingrid_db_version' LIMIT 1 ;
...
UPDATE folder_menu SET `ELEMENT_ORDER` = '9' WHERE `folder_menu`.`OPTIONS` ="/administration" AND `folder_menu`.`PARENT_ID` = (SELECT temp_value FROM ingrid_temp WHERE temp_key = 'main_menu_id') LIMIT 1 ;
UPDATE folder_menu SET `ELEMENT_ORDER` = '10' WHERE `folder_menu`.`OPTIONS` ="/Administrative" AND `folder_menu`.`PARENT_ID` = (SELECT temp_value FROM ingrid_temp WHERE temp_key = 'main_menu_id') LIMIT 1 ;
UPDATE folder_menu SET `ELEMENT_ORDER` = '11' WHERE `folder_menu`.`OPTIONS` ="/mdek" AND `folder_menu`.`PARENT_ID` = (SELECT temp_value FROM ingrid_temp WHERE temp_key = 'main_menu_id') LIMIT 1 ;
...
INSERT INTO ingrid_cms (item_key, item_description, item_changed_by) VALUES ('portal.teaser.shortcut', 'Anwendungen', 'admin');
INSERT INTO ingrid_cms_item (fk_ingrid_cms_id, item_lang, item_title, item_value, item_changed_by) VALUES ((SELECT id FROM ingrid_cms WHERE item_key  = 'portal.teaser.shortcut'), 'de', 'Anwendungen', '', 'admin');
INSERT INTO ingrid_cms_item (fk_ingrid_cms_id, item_lang, item_title, item_value, item_changed_by) VALUES ((SELECT id FROM ingrid_cms WHERE item_key  = 'portal.teaser.shortcut'), 'en', 'Application', '', 'admin');

INSERT INTO ingrid_cms (item_key, item_description, item_changed_by) VALUES ('portal.teaser.shortcut.query', 'Schnellsuche', 'admin');
INSERT INTO ingrid_cms_item (fk_ingrid_cms_id, item_lang, item_title, item_value, item_changed_by) VALUES ((SELECT id FROM ingrid_cms WHERE item_key  = 'portal.teaser.shortcut.query'), 'de', 'Schnellsuche', '', 'admin');
INSERT INTO ingrid_cms_item (fk_ingrid_cms_id, item_lang, item_title, item_value, item_changed_by) VALUES ((SELECT id FROM ingrid_cms WHERE item_key  = 'portal.teaser.shortcut.query'), 'en', 'Quick Search', '', 'admin');
