-- DB Version erhöhen
UPDATE ingrid_lookup SET `item_value` = '3.2.0c', `item_date` = NOW() WHERE `ingrid_lookup`.`item_key` ='ingrid_db_version' LIMIT 1 ;

-- Update indexes on portal preferences to increase the user manager performance
-- ======================

ALTER TABLE `prefs_property_value` ADD INDEX ( `NODE_ID` ) ;
ALTER TABLE `prefs_property_value` ADD INDEX ( `PROPERTY_NAME` );
ALTER TABLE `prefs_node` ADD INDEX ( `FULL_PATH` );
ALTER TABLE `security_permission` ADD INDEX ( `CLASSNAME` );
...
-- add cms key
INSERT INTO ingrid_cms (item_key, item_description, item_changed_by) VALUES ('portal.cms.portlet.0', 'CMSPortlet0', 'admin');
INSERT INTO ingrid_cms_item (fk_ingrid_cms_id, item_lang, item_title, item_value, item_changed_by) VALUES ((SELECT id FROM ingrid_cms WHERE item_key  = 'portal.cms.portlet.0'), 'de', 'CMSPortlet0', '', 'admin');
INSERT INTO ingrid_cms_item (fk_ingrid_cms_id, item_lang, item_title, item_value, item_changed_by) VALUES ((SELECT id FROM ingrid_cms WHERE item_key  = 'portal.cms.portlet.0'), 'en', 'CMSPortlet0', '', 'admin');

-- add cms key
INSERT INTO ingrid_cms (item_key, item_description, item_changed_by) VALUES ('portal.cms.portlet.1', 'CMSPortlet1', 'admin');
INSERT INTO ingrid_cms_item (fk_ingrid_cms_id, item_lang, item_title, item_value, item_changed_by) VALUES ((SELECT id FROM ingrid_cms WHERE item_key  = 'portal.cms.portlet.1'), 'de', 'CMSPortlet1', '', 'admin');
INSERT INTO ingrid_cms_item (fk_ingrid_cms_id, item_lang, item_title, item_value, item_changed_by) VALUES ((SELECT id FROM ingrid_cms WHERE item_key  = 'portal.cms.portlet.1'), 'en', 'CMSPortlet1', '', 'admin');

-- add cms key
INSERT INTO ingrid_cms (item_key, item_description, item_changed_by) VALUES ('portal.cms.portlet.2', 'CMSPortlet2', 'admin');
INSERT INTO ingrid_cms_item (fk_ingrid_cms_id, item_lang, item_title, item_value, item_changed_by) VALUES ((SELECT id FROM ingrid_cms WHERE item_key  = 'portal.cms.portlet.2'), 'de', 'CMSPortlet2', '', 'admin');
INSERT INTO ingrid_cms_item (fk_ingrid_cms_id, item_lang, item_title, item_value, item_changed_by) VALUES ((SELECT id FROM ingrid_cms WHERE item_key  = 'portal.cms.portlet.2'), 'en', 'CMSPortlet2', '', 'admin');

-- delete temporary table
DROP TABLE IF EXISTS ingrid_temp;

-- change ingrid_rss_source.description from VARCHAR( 255 ) to VARCHAR( 1023 )
ALTER TABLE `ingrid_rss_source` CHANGE `description` `description` VARCHAR( 1023 );
ALTER TABLE `ingrid_rss_store` CHANGE `title` `title` VARCHAR( 1023 );
ALTER TABLE `ingrid_rss_store` CHANGE `author` `author` VARCHAR( 1023 );
