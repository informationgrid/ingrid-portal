-- DB Version erhöhen
UPDATE `ingrid_lookup` SET `item_value` = '3.0.0' WHERE `ingrid_lookup`.`item_key` ='ingrid_db_version' LIMIT 1 ;
...
DELETE FROM `fragment` WHERE `name` = 'ingrid-portal-mdek::MdekQuickViewPortlet' LIMIT 1 ;
DELETE FROM `portlet_definition` WHERE `name` = 'MdekQuickViewPortlet' LIMIT 1 ;
DELETE FROM `portlet_entity` WHERE `portlet_name` = 'MdekQuickViewPortlet' LIMIT 1 ;
