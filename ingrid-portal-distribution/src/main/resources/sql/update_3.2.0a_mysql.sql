-- DB Version erhöhen
UPDATE ingrid_lookup SET `item_value` = '3.2.0a', `item_date` = NOW() WHERE `ingrid_lookup`.`item_key` ='ingrid_db_version' LIMIT 1 ;
...
-- PARENT_ID of weather teaser, was page_id before not id of parent layout fragment (worked due to same id of page and parent layout fragment) !
    ('WEATHER_FRAGMENT_ID', (SELECT FRAGMENT_ID FROM fragment WHERE NAME="ingrid-portal-apps::WeatherTeaser" AND PARENT_ID=(SELECT PAGE_ID FROM page WHERE PATH="/default-page.psml" ))),
    ('PAGE_ID', (SELECT PAGE_ID FROM page WHERE PATH="/default-page.psml"));
