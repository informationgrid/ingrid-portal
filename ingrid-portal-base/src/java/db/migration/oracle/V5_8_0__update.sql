-- DB Version
UPDATE ingrid_lookup SET item_value = '5.8.0', item_date = SYSDATE WHERE ingrid_lookup.item_key ='ingrid_db_version';
...
INSERT INTO page (PAGE_ID, CLASS_NAME, PARENT_ID, PATH, NAME, VERSION, TITLE, SHORT_TITLE, IS_HIDDEN, SKIN, DEFAULT_LAYOUT_DECORATOR, DEFAULT_PORTLET_DECORATOR, SUBSITE, USER_PRINCIPAL, ROLE_PRINCIPAL, GROUP_PRINCIPAL, MEDIATYPE, LOCALE, EXT_ATTR_NAME, EXT_ATTR_VALUE, OWNER_PRINCIPAL) VALUES 
    ((SELECT max_key FROM ojb_hl_seq where tablename='SEQ_PAGE') , 'org.apache.jetspeed.om.page.impl.PageImpl' , (SELECT folder_id FROM folder where path='/'), '/rsspage.psml', 'rsspage.psml', NULL, 'ingrid.page.rsspage.tooltip', 'ingrid.page.rsspage', '1', 'orange', 'ingrid', 'ingrid-teaser', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);

-- add fragments to rsspage.psml
INSERT INTO fragment (FRAGMENT_ID ,CLASS_NAME ,PARENT_ID ,PAGE_ID ,NAME ,TITLE ,SHORT_TITLE ,TYPE ,SKIN ,DECORATOR ,STATE ,PMODE ,LAYOUT_ROW ,LAYOUT_COLUMN ,LAYOUT_SIZES ,LAYOUT_X ,LAYOUT_Y ,LAYOUT_Z ,LAYOUT_WIDTH ,LAYOUT_HEIGHT ,OWNER_PRINCIPAL) VALUES 
    ...
    ((SELECT max_key+1 FROM ojb_hl_seq where tablename='SEQ_FRAGMENT'), 'org.apache.jetspeed.om.page.impl.FragmentImpl', (SELECT max_key FROM ojb_hl_seq where tablename='SEQ_FRAGMENT'), NULL , 'ingrid-portal-apps::RSSPagePortlet', NULL , NULL , 'portlet', NULL , 'ingrid-teaser', NULL , NULL , '0', '0', NULL , '-1', '-1', '-1', '-1', '-1', NULL);

-- add page_constraints_ref to rsspage.psml
...
INSERT INTO page_metadata (METADATA_ID ,PAGE_ID ,NAME ,LOCALE ,VALUE) VALUES 
    ((SELECT max_key    FROM ojb_hl_seq where tablename='SEQ_PAGE_METADATA'),(SELECT PAGE_ID FROM page WHERE PATH='/rsspage.psml'), 'meta_title', 'de', 'ingrid.page.rsspage.meta.title');

-- add meta_description
INSERT INTO page_metadata (METADATA_ID ,PAGE_ID ,NAME ,LOCALE ,VALUE) VALUES 
    ((SELECT max_key+1  FROM ojb_hl_seq where tablename='SEQ_PAGE_METADATA'),(SELECT PAGE_ID FROM page WHERE PATH='/rsspage.psml'), 'meta_descr', 'de', 'ingrid.page.rsspage.meta.description');

-- add meta_keywords
INSERT INTO page_metadata (METADATA_ID ,PAGE_ID ,NAME ,LOCALE ,VALUE) VALUES 
    ((SELECT max_key+2  FROM ojb_hl_seq where tablename='SEQ_PAGE_METADATA'),(SELECT PAGE_ID FROM page WHERE PATH='/rsspage.psml'), 'meta_keywords', 'de', 'ingrid.page.rsspage.meta.keywords');

-- add rsspage to footer menu
...
INSERT INTO folder_menu (MENU_ID, CLASS_NAME, PARENT_ID, ELEMENT_ORDER, NAME, TITLE, SHORT_TITLE, TEXT, OPTIONS, DEPTH, IS_PATHS, IS_REGEXP, PROFILE, OPTIONS_ORDER, SKIN) VALUES
    ((SELECT max_key FROM ojb_hl_seq where tablename='SEQ_FOLDER_MENU'), 'org.apache.jetspeed.om.folder.impl.FolderMenuOptionsDefinitionImpl', (SELECT temp_value FROM ingrid_temp WHERE temp_key = 'footer_menu_id'), '6', NULL, NULL, NULL, NULL, '/rsspage.psml', '0', '0', '0', NULL, NULL, NULL);

-- add cms rsspage and query shortcut
INSERT INTO ingrid_cms (item_key, item_description, item_changed_by) VALUES ('portal.rsspage', 'RSS Seite', 'admin');
INSERT INTO ingrid_cms_item (fk_ingrid_cms_id, item_lang, item_title, item_value, item_changed_by) VALUES ((SELECT id FROM ingrid_cms WHERE item_key  = 'portal.rsspage'), 'de', 'RSS Seite', 'Verwenden Sie die Administration des Portals den Text der RSS Seite zu pflegen und darzustellen.', 'admin');
INSERT INTO ingrid_cms_item (fk_ingrid_cms_id, item_lang, item_title, item_value, item_changed_by) VALUES ((SELECT id FROM ingrid_cms WHERE item_key  = 'portal.rsspage'), 'en', 'RSS Page', 'Verwenden Sie die Administration des Portals den Text der RSS Seite zu pflegen und darzustellen.', 'admin');
