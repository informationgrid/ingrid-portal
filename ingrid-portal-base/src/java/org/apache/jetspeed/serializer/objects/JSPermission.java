...
public class JSPermission
{
    ...
    public String getClassForType(String type)
    {
        if ((type == null) || (type.length() == 0) || (type == TYPE_UNKNOWN)) {
            if (permissionClass != null && permissionClass.length() > 0) {
    ...
    public Permission getPermission() {
        Permission p = getPermissionForType();
        if ( p != null) {
            return p;
        }
        if (permissionClass != null && permissionClass.length() > 0) {
            ...
            classParm[1] = String.class;
            Object [] objectParm = new Object[2];
            objectParm[0] = this.resource;
            objectParm[1] = this.actions;
            
            try {
              Class cl = Class.forName(permissionClass);
              java.lang.reflect.Constructor co = cl.getConstructor(classParm);
              return (Permission)co.newInstance(objectParm);
              }
            catch (Exception e) {
              e.printStackTrace();
            }            
    ...
    private static final XMLFormat XML = new XMLFormat(JSPermission.class)
    {
                ...
                throws XMLStreamException
        {
            try
            {
                ...
                xml.setAttribute("class", g.getPermissionClass());
        ...
        public void read(InputElement xml, Object o)
        {
            try
            {
                ...
                g.permissionClass = StringEscapeUtils.unescapeHtml(xml.getAttribute("class", "type_unknown"));
